...
class TagsField extends Component {
    ...
    boldQuery = (str) => {
        ...
        const n = escaped.toUpperCase();
        const q = escapeHtml(this.lastInputValue).toUpperCase();
        const x = n.indexOf(q);
        if (!q || x === -1) return escaped;
        const l = q.length;
        return `${escaped.substr(0, x)}<b>${escaped.substr(x, l)}</b>${escaped.substr(x + l)}`;
    }
